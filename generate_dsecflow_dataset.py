...
def events_to_voxel_grid(self, evs, width, height, num_bins, use_pol=True):
        """
        Build a voxel grid with bilinear interpolation in the time domain from a set of events.
...
if __name__ == '__main__':
    ...
    event_frame_save_path = "I:/DSEC/" + name + "/event_frame"
    if not os.path.exists(event_frame_save_path):
        os.makedirs(event_frame_save_path)
    
    event_voxel_save_path = "I:/DSEC/" + name + "/event_voxel"
    if not os.path.exists(event_voxel_save_path):
        os.makedirs(event_voxel_save_path)
    
    image_rendering_save_path = "I:/DSEC/" + name + "/images_rendering"
    if not os.path.exists(image_rendering_save_path):
        os.makedirs(image_rendering_save_path)
    
    
    ev_data_file = os.path.join("I:/DSEC", name + '/events/events.h5')
    ev_location = h5py.File(str(ev_data_file), 'r')
    events_slicer = EventSlicer(ev_location)
    time_delta = 20000
    # 读取txt文件
    frame_id = 0
    with open("I:/DSEC/" + name + '/timestamps.txt', 'r') as file:
        ...
        for line in lines:
            ...
            save_name = os.path.join(event_frame_save_path, frame_name+'.png')
            ...
            for i in range(evs_voxel.shape[0]):
                save_name = os.path.join(event_voxel_save_path, frame_name+'_'+str(i).zfill(2)+'.png')
                ...
                cv2.imwrite(save_name, uint8_image)
                    
            # 将图像和事件帧混搭覆盖
            img_raw = cv2.imread("I:/DSEC/"+name +"/images_rectify/" + frame_name + ".png")
            event_image_ = cv2.imread(os.path.join(event_frame_save_path, frame_name+'.png'))
            
            lower_white = np.array([220, 220, 220])
            upper_white = np.array([255, 255, 255])
            mask = cv2.inRange(event_image_, lower_white, upper_white)
            event_image_[mask != 0] = [0, 0, 0]

            overlay_image = cv2.add(img_raw, event_image_)
            save_name = os.path.join(image_rendering_save_path, frame_name+'.png')
            cv2.imwrite(save_name, overlay_image)
