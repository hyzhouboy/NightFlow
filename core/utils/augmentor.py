...
class EventFlowAugmentor:
    ...
    def spatial_transform(self, img1, img2, event1, event2, flow, valid):
        pad_t = 0
        pad_b = 0
        pad_l = 0
        pad_r = 0
        ...
        if pad_b != 0 or pad_r != 0:
            img1 = np.pad(img1, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
            img2 = np.pad(img2, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
            event1 = np.pad(event1, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
            event2 = np.pad(event2, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
            flow = np.pad(flow, ((pad_t, pad_b), (pad_l, pad_r), (0, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
            valid = np.pad(valid, ((pad_t, pad_b), (pad_l, pad_r)), 'constant', constant_values=((0, 0), (0, 0)))
        ...
        #     img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        #     img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        #     event1 = cv2.resize(event1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        #     event2 = cv2.resize(event2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
        #     flow, valid = self.resize_sparse_flow_map(flow, valid, fx=scale_x, fy=scale_y)
        ...
        # crop image
        # print()
        y0 = np.random.randint(margin_y, img1.shape[0] - self.crop_size[0] - margin_y)
        x0 = np.random.randint(margin_x, img1.shape[1] - self.crop_size[1] - margin_x)

        img1 = img1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        img2 = img2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        event1 = event1[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        event2 = event2[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        valid = valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]

        return img1, img2, event1, event2, flow, valid


    def __call__(self, img1, img2, event1, event2, flow, valid):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        # img1, img2, flow, valid = self.spatial_transform(img1, img2, flow, valid)
        
        img1, img2, event1, event2, flow, valid = self.spatial_transform(img1, img2, event1, event2, flow, valid)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        event1 = np.ascontiguousarray(event1)
        event2 = np.ascontiguousarray(event2)
        flow = np.ascontiguousarray(flow)
        valid = np.ascontiguousarray(valid)
